...
class ScaledDotProductAttention:
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        scaled_dot_product = np.matmul(Q, KT) / d_k
    ...
    def backward(self, d_output):
        ...
        d_scaled_dot_product = np.matmul(d_output, np.swapaxes(self.V, -1, -2))
        d_scaled = self.softmax.backward(d_scaled_dot_product)
        
        # Scale gradients by sqrt(d_k)
        d_scaled_dot_product = d_scaled / np.sqrt(self.Q.shape[-1])
