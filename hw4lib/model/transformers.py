...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        x_enc, x_enc_lengths = self.source_embedding(padded_sources, source_lengths) # Speech embedding
        ...
        if not self.skip_encoder_pe:
            x_enc = self.positional_encoding(x_enc) # Positional encoding
        
        # TODO: Apply dropout
        x_enc = self.dropout(x_enc) # Dropout

        # TODO: Create source padding mask on the same device as the input
        pad_mask_src = PadMask(x_enc, x_enc_lengths) # Source padding mask
        ...
        # TODO: Apply normalization
        x_enc = self.encoder_norm(x_enc) # Encoder normalization
        # TODO: Project to CTC logits
        ctc_logits = self.ctc_head(x_enc.transpose(0, 1)) # CTC head
    ...
    def decode(
        ...
        pad_mask_tgt = None
        if target_lengths is not None:
            pad_mask_tgt = PadMask(padded_targets, target_lengths) # Target padding mask
        ...
        if not self.skip_decoder_pe:
            x_dec = self.positional_encoding(x_dec) # Positional encoding

        # TODO: Apply dropout
        x_dec = self.dropout(x_dec) # Dropout
