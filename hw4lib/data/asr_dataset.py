...
class ASRDataset(Dataset):
    def __init__(
        ...
        self.fbank_dir   = os.path.join(config['root'], partition, "fbank") # replace with fbank dir
        ...
        if subset_size is not None:
            self.fbank_files = self.fbank_files[:int(subset_size*len(self.fbank_files))]
        ...
        if self.partition != "test-clean":
            ...
            self.text_dir   = os.path.join(config['root'], partition, "text") # replace with text dir
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [f[0].T for f in batch] # B x T x F
        ...
        feat_lengths =  torch.LongTensor([f.size(0) for f in batch_feats]) # B
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [f[1] for f in batch] # B x T
            batch_golden       = [f[2] for f in batch] # B x T
            ...
            transcript_lengths = torch.LongTensor([f.size(0) for f in batch_shifted]) # B 
